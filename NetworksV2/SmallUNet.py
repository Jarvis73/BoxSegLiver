...
class _ModelConfig(object):
    ...
    config[3] = {
        "conv_e0": {"conv1": {"out":   64, "kernel": (3, 3), "stride": (1, 1), "dilated": 1},
                    "conv2": {"out":   64, "kernel": (3, 3), "stride": (1, 1), "dilated": 1}},
        "conv_e1": {"conv1": {"out":  128, "kernel": (3, 3), "stride": (2, 2), "dilated": 1},
                    "conv2": {"out":  128, "kernel": (3, 3), "stride": (1, 1), "dilated": 1}},
        "conv_e2": {"conv1": {"out":  256, "kernel": (3, 3), "stride": (2, 2), "dilated": 1},
                    "conv2": {"out":  256, "kernel": (3, 3), "stride": (1, 1), "dilated": 1}},
        "conv_e3": {"conv1": {"out":  512, "kernel": (3, 3), "stride": (2, 2), "dilated": 1},
                    "conv2": {"out":  512, "kernel": (3, 3), "stride": (1, 1), "dilated": 1}},

        "bridge":  {"conv1": {"out": 1024, "kernel": (3, 3), "stride": (1, 1), "dilated": 2},
                    "conv2": {"out": 1024, "kernel": (3, 3), "stride": (1, 1), "dilated": 2}},
...
class SmallUNet(base.BaseNet):
    def __init__(self, args, name=None):
        """ Don't create ops/tensors in __init__() """
        super(SmallUNet, self).__init__(args)
    ...
    def _build_network(self, *args, **kwargs):
        ...
        with tf.variable_scope(self.name):
                    ...
                    if block_key.startswith("conv_e") or block_key == "bridge":
                        for layer_key, layer in block.items():
                            x = slim.conv2d(x, num_outputs=round(layer["out"] * c),
                                            ...
                                            rate=layer["dilated"],
                                            scope=layer_key)
                        end_pts[block_key] = {"x": x}
                    elif block_key.startswith("conv_d"):
                        for layer_key, layer in block.items():
                            if layer_key == "up":
                                enc_key = block_key.replace("d", "e")
                                x = slim.conv2d_transpose(x, num_outputs=round(layer["out"] * c),
                                                          kernel_size=layer["kernel"],
                                                          stride=layer["stride"],
            ...
            logits = slim.conv2d(x, self.num_classes,
                                 kernel_size=1,
                                 normalizer_fn=None,
                                 activation_fn=None,
                                 scope="logits")
