...
class CustomEstimator(object):
    ...
    def __init__(self, model_fn, model_dir=None, config=None, params=None, warm_start_from=None):
        ...
        self.double_dataloader_modes = self._params.get("double_dataloader_modes", None)
        if self.double_dataloader_modes and len(self.double_dataloader_modes) != 2:
            raise ValueError("double_dataloader_modes need a list of 2 elements for specifying input_fn modes")
    ...
    def evaluate(self,
        ...
        checkpoint_path = self.checkpoint_path(checkpoint_path, latest_filename)
        results = evaluator.run(input_fn,
                                hooks=hooks,
        ...
        try:
            ...
            while True:
                if steps and counter >= steps:
                    break
                preds_evaluated = session.run(predictions, {self.handler: self.dataset_handle_hook.eval_handle})
                if not yield_single_examples:
                    yield preds_evaluated
                elif not isinstance(predictions, dict):
                    for pred in preds_evaluated:
    ...
    def checkpoint_path(self, checkpoint_path, latest_filename):
    ...
    def _get_features_and_labels_from_double_dataloader(self, input_fn, handler):
        return parse_input_fn_result(
            train_with_eval=True,
            result=[self._call_input_fn(input_fn, self.double_dataloader_modes[0]),
                    self._call_input_fn(input_fn, self.double_dataloader_modes[1])],
            handler=handler)
    ...
    def _get_iterator_from_double_dataloader(self, input_fn, handler, distribution=None):
        if distribution is not None:
            result = [
                distribution.distribute_dataset(
                    lambda: self._call_input_fn(input_fn, self.double_dataloader_modes[0])),
                distribution.distribute_dataset(
                    lambda: self._call_input_fn(input_fn, self.double_dataloader_modes[1]))
            ]
        else:
            result = [
                self._call_input_fn(input_fn, self.double_dataloader_modes[0]),
                self._call_input_fn(input_fn, self.double_dataloader_modes[1])
            ]
    ...
    def _train_model_default(self, input_fn, hooks, saving_listeners):
        ...
        with ops.Graph().as_default() as g:
            ...
            if self.double_dataloader_modes:
                ...
                features, labels, self.train_iterator, self.eval_iterator, input_hooks = (
                    self._get_features_and_labels_from_double_dataloader(
    ...
    def _train_model_distributed(self, strategy, input_fn, hooks, saving_listeners):
            ...
            with strategy.scope():
                ...
                if self.double_dataloader_modes:
                    ...
                    iterator, self.train_iterator, self.eval_iterator, input_hooks = (
                        self._get_iterator_from_double_dataloader(input_fn, self.handler, strategy))
                else:
                    self.handler, self.train_iterator, self.eval_iterator = None, None, None
    ...
    def _train_with_estimator_spec(self, estimator_spec, worker_hooks, hooks,
        ...
        if self._config.log_step_count_steps is not None:
            ...
            lr_collections = ops.get_collection(CustomKeys.LEARNING_RATE)
            if len(lr_collections) > 0:
                tensors["lr"] = lr_collections[0]
            tensors.update({key.replace("/", ""): val
                            for key, val in estimator_spec.predictions.items() if "/" in key})
            worker_hooks.append(
                hooks_lib.LoggingTensorWithSpeedFormatterHook(
        ...
        if self.double_dataloader_modes:
            self.dataset_handle_hook = hooks_lib.IteratorStringHandleHook(self.train_iterator,
                                                                          self.eval_iterator)
        ...
        with training.MonitoredTrainingSession(
                ...
                config=self._session_config,
                log_step_count_steps=0) as mon_sess:
            loss = None
