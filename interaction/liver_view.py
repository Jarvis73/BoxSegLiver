...
class SegViewerAdapter(object):
    ...
    def get_slice2(self, ind, ges):
        ...
        if sind in self.interaction[spid] and len(self.interaction[spid][sind]["center"]) > 0:
            ind = (ind - self.get_min_idx(ges)) % self.vol.shape[ges - 1]
            img = self.resized_image_single(self.vol, ges, ind)
            center = np.array(self.interaction[spid][sind]["center"])[:, ::-1]
            stddev = np.array(self.interaction[spid][sind]["stddev"])[:, ::-1]
    ...
    def get_file_list(self):
        ...
        if not self.table:
            for path in self.data_dir.glob("*.nii*"):
    ...
    def update_interaction(self, ind, center, axes):
        ...
        if sind in self.interaction[spid]:
            self.interaction[spid][sind]["center"].append(list(center))
            self.interaction[spid][sind]["stddev"].append(stddev)
        else:
            self.interaction[spid][sind] = {"center": [list(center)], "stddev": [stddev]}

    def pop_interaction(self, ind):
        sind = str(ind)
        spid = str(self.pid)
        if sind in self.interaction[spid]:
            self.interaction[spid][sind]["center"].pop()
            self.interaction[spid][sind]["stddev"].pop()
