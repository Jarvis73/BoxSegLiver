...
if [[ "$TASK" == "train" ]]; then
   PYTHONPATH="${PROJECT_DIR}" PYTHONNOUSERSITE=True CUDA_VISIBLE_DEVICES=${GPU_ID} \
        ...
        --classes NF \
        --test_fold 0 \
        --im_depth 10 --im_height 192 --im_width 112 --im_channel 1 \
        --random_flip 7 \
        --num_of_total_steps 300000 \
        ...
        --use_spatial --local_enhance --stddev 1. 5. 5. \
        --save_interval 50000 \
        -ds \
        $@
elif [[ "$TASK" == "eval" ]]; then
    PYTHONPATH="${PROJECT_DIR}" PYTHONNOUSERSITE=True CUDA_VISIBLE_DEVICES=${GPU_ID} \
        ...
        --classes NF \
        --test_fold 0 \
        --im_depth -1 --im_height 480 --im_width 160 --im_channel 1 \
        --random_flip 7 \
        --batch_size 1 \
        --normalizer instance_norm \
        --eval_mirror \
